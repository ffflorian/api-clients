...
export class RequestService {
  private apiUrl = 'https://c3nav.de/api/';
  ...
  public head<T>(endpoint: string, parameters?: RequestOptions): Promise<T> {
    return this.request<T>('head', endpoint, parameters);
  }

  public options<T>(endpoint: string, parameters?: RequestOptions): Promise<T> {
    return this.request<T>('options', endpoint, parameters);
  }
